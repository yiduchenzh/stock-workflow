...
def check_mtf_resonance(kline_df):
    """周线趋势(真实周K)→日线MACD/RSI→60分入场(5日代理)"""
    ...
    # 第一重: 周线趋势 (用最近12周=60日做真实周K聚合)
    weekly_kl = _daily_to_weekly(kline_df)
    ...
    score = weekly_score * 0.4 + (30 if daily_ok else 10) + div_bonus + kdj_bonus + (25 if trigger else 5)
    return {
        "resonance": score >= 60, "score": min(score, 100),
        "detail": f"周{weekly_score}日{'OK' if daily_ok else 'WAIT'}60{'↑' if trigger else '→'}",
        "weekly_score": weekly_score, "daily_macd": macd_up, "trigger": trigger,
